    ...
    if (!state.countyError && !state.sub_countyError && !state.nameError) {
      ...
      UpdateSchoolAsync()
        .then(data => {
          ...
          this.setState({
            open: true,
            school: parseSchool({ ...data.school, addedBy: state.addedBy }),
            oldSchool: parseSchool({ ...data.school, addedBy: state.addedBy }),
            updating: false,
