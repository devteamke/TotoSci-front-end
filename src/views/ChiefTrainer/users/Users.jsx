...
class AllUsers extends React.Component  {
	...
	_fetchUsers = () => {
	    ...
	    let data = {
            ...
            await (await fetch(`${globals.BASE_URL}/api/${this.props.global.user.role}/all`, {
              method: 'post',
								...
								<tr>
								  ...
								  <MDBTableBody>
									  {state.users.map(user =>
										 <tr key={user._id} onClick={()=>{	this.props.history.push({pathname: `/${this.props.global.user.role}/users/single`, data:user})}} style={{cursor: 'pointer'}}>
											  <td>{user._id}</td>
											  <td>{user.email}</td>
