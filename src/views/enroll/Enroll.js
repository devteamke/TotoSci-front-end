...
class AddUser  extends React.Component  {
   ...
   handleSubmit= ()=> {
		...
		let state = this.state;
	     ...
	     const pfnameError = validate('pfname', state.pfname===''?null:state.pfname);
	     ...
	     const ponameError = validate('poname', state.poname===''?null:state.poname);
      ...
      () => {
        ...
        if ( !emailError && !fnameError && !snameError && !snameError && !onameError && !residenceError && !phoneError  ) {
          ...
          const AddAsync = async () =>
            await (await fetch(`${globals.BASE_URL}/api/users/student/new`, {
              method: 'post',
        ...
        <GridItem xs={12} sm={12} md={8}>
          <Card>
            <CardHeader color="primary">
              <h4 className={classes.cardTitleWhite}>Enroll student to school</h4>
            ...
            <h4> Parent / Guardian info </h4>
                  ...
                  <MDBInput
					...
					group
					value={state.pfname}
					 onChange={(event)=>{ this.setState({pfname:event.target.value})}}
					onBlur={()=>this.setState({pfnameError:validate('pfname', state.pfname==''?null:state.pfname)})}
					error="Whoops!"
					success="right"
				  />
					<p style={{color:'red', fontSize:'0.8rem', textAlign:'center'}}>{state.pfnameError}</p>
                </GridItem>
			...
			<GridItem xs={12} sm={12} md={6}>
                  <MDBInput
					...
					group
					value={state.poname}
				    onChange={(event)=>{ this.setState({poname:event.target.value})}}
				    onBlur={()=>this.setState({ponameError:validate('oname', state.poname==''?null:state.oname)})}
					error="Whoops!"
					success="right"
				  />
					<p style={{color:'red', fontSize:'0.8rem', textAlign:'center'}}>{state.ponameError}</p>
                </GridItem>
                ...
                <GridItem xs={12} sm={12} md={4}>
				 ...
				 {state.addingUser?  <div className="spinner-grow text-info" role="status" style={{marginBottom:'15px'}}>
				  ...
				  </div> :<MDBBtn onClick={this.handleSubmit}>Enroll </MDBBtn>}
				</div>
				   </GridItem>
