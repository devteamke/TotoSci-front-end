...
class Single extends React.Component{
	 constructor(props) {
		...
		if(!user){
			this.props.history.push(`/${this.props.global.user.role}/users`)
			return;
		}
		 user= parseUser(user);
      ...
      () => {
		...
		this.setState({updating:true})
          ...
          const UpdatePassAsync = async () =>
            await (await fetch(`${globals.BASE_URL}/api/${this.props.global.user.role}/password`, {
              method: 'PATCH',
			...
			this.setState({savingInfo:true},()=>{
				let data = {
				   ...
				   const SaveAsync = async () =>
					await (await fetch(`${globals.BASE_URL}/api/${this.props.global.user.role}/save_profile`, {
					  method: 'PATCH',
					  mode: 'cors', // no-cors, cors, *same-origin
					  cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
					  credentials: 'same-origin', // include, *same-origin, omit
	...
	this.setState({deleteModal:!this.state.deleteModal, deleting:true});
	 let data = {
          ...
          const DeleteAsync = async () =>
            await (await fetch(`${globals.BASE_URL}/api/${this.props.global.user.role}/requestDelete`, {
              method: 'DELETE',
              mode: 'cors', // no-cors, cors, *same-origin
              cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
              credentials: 'same-origin', // include, *same-origin, omit
